...
def as_si(x, ndp):
    """ Convert number to latex-style x10 scientific notation string"""
    s = f'{x:0.{ndp:d}e}'
    m, e = s.split('e')
    return rf'{m:s}\times 10^{{{int(e):d}}}'
...
def generate_dir_names(dataset, args, make=True):
    suffix = f'{args.optim}_Eps{args.lip_eps}_B{args.lip_calls}'
...
def plot_decision(ax, xx, yy, Z, X_train, X_test, y_train, y_test, score, title=None):
    ...
    if title:
        ax.set_title(title)
    ax.text(xx.max() - .3, yy.min() + .3, f'{score:.2f}'.lstrip('0'),
...
def plot_dependencies(dictionary_values,
    ...
    if np.sum(np.array(list(dictionary_values.values()))) == 0.0:
    ...
    if show_table and ax_table:
        cell_text = [[f'{v:1.{digits}f}'] for v in x]
...
def plot_importances(explainer, x, y, ax=None, feat=0):
    ...
    ax.set_title(f'Importances of Feature: {feat}')
...
def barchart_insert(coefs, ax=None, labels=None):
    ...
    cmap = matplotlib.colormaps.get_cmap('tab10') if hasattr(matplotlib, 'colormaps') else matplotlib.cm.get_cmap('tab10')
    # color = ['r','b'])
...
def plot_relevance_explainer(explainer, X, y, line_scale=1, plot_dims=None, explain='predicted',
    ...
    for i, center in enumerate(X):
        ...
        #print(posx, posy)
        # posx, posy, width, height - all in axes-relative coordinates
        # Use modern inset_axes API instead of deprecated InsetPosition
        iax = ax.inset_axes([posx, posy, 0.05, 0.05])
...
def plot_attribution_stability(model, explainers, input, pert_type='gauss', noise_level=0.5,
    ...
    for i, x in enumerate(inputs):
        ...
        ax[pos].axis('off')
        if i == 0:
            ax[pos].set_title('Original', fontsize=20)
        else:
            if show_proba:
                title = f'P({preds[i]})={outputs[i, preds[i]]:0.4e}'
            else:
                title = f'Perturbation {i}'
    ...
    for i, (method, att) in enumerate(atts.items()):
        ...
        for j in range(samples + 1):
            ...
            if j == 0:
                ...
                axij.set_title(rf"$\hat{{L}}={d:4.2f}$", fontsize=20)
...
def lipschitz_argmax_plot(x, y, att_x, att_y, lip=None, ax=None, save_path=None):
    ...
    if lip:
        ax[1, 1].set_title(rf'$\delta={lip:4.2e}$')
    if save_path:
        plt.savefig(save_path, bbox_inches='tight', format='pdf', dpi=300)
    plt.show(block=False)
...
def lipschitz_feature_argmax_plot(x, y, att_x, att_y, pred_x = None, pred_y =None,
    ...
    if pred_x is not None:
        ...
        xtext = rf'$P(y={lab_x})={pred_x[lab_x]:.2f}$'
    if pred_y is not None:
        lab_y = int(np.argmax(pred_y))
        ytext = rf'$P(y={lab_y})={pred_y[lab_y]:.2f}$'
...
def attrib_consistency_plot(df, xvar, yvar, xerrvar=None, yerrvar=None,
    ...
    if show_correlation:
        ...
        if p < 0.01 and sci_pval:
            legend_label = rf"${corrsymbol}: {corr:2.2f}$" + "\n" + rf"p-value: ${as_si(p,1):s}$"
        else:
            legend_label = rf"${corrsymbol}: {corr:2.2f}$" + "\n" + rf"p-value: ${p:2.2f}$"
    else:
        legend_label = None
    ...
    ax.set_title(r'Attribution Score Consistency' + (f': {title}' if title else ''), fontsize=title_fontsize)
