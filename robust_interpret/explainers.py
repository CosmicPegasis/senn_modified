...
def make_keras_picklable():
    """Make Keras models picklable. Handles both TensorFlow 2.x and standalone Keras."""
    def __getstate__(self):
        model_str = b""
        with tempfile.NamedTemporaryFile(suffix='.hdf5', delete=True, mode='wb') as fd:
            try:
                # Try TensorFlow 2.x Keras
    ...
    # Get the Model class - handle both import styles
    try:
...
class explainer_wrapper(object):
    ...
    def estimate_dataset_lipschitz(self, dataset, continuous=True, eps=1,
        ...
        if n_jobs > 1:
            ...
            for x in dataset_filt:
                ...
                Lips.append(l)
        print(f'Missed points: {sum(x is None for x in Lips)}/{len(dataset_filt)}')
    ...
    def estimate_discrete_dataset_lipschitz(self, dataset, eps = None, top_k = 1,
        ...
        if eps is not None:
            ...
            print(f'Number of zero denom distances: {total - nonzero} ({100*(total-nonzero)/total:4.2f}%)')
...
class shap_wrapper(explainer_wrapper):
    ...
    def __init__(self, model, shap_type, link, mode, multiclass=False, feature_names=None,
        ...
        print(f'Initializing {shap_type} SHAP explainer wrapper')
    ...
    def __call__(self, x, y=None,  x_raw=None, return_dict=False, show_plot=False):
        ...
        elif y is not None and hasattr(y, 'ndim') and y.ndim > 1:
            # Means y is given in one-hot format
            y = np.argwhere(y)[:, 1]
        elif isinstance(y, (int, np.integer)) or (hasattr(y, 'ndim') and y.ndim == 0):
            # y is a scalar (int or numpy scalar) - convert to list
            y = [int(y)] * x.shape[0]
        elif hasattr(y, '__len__'):
            # y is already a list/array - convert to list and ensure correct length
            y = [int(y[i]) if i < len(y) else int(y[0]) for i in range(x.shape[0])]
        else:
            # Fallback: try to convert to list
            y = [int(y)] * x.shape[0]

        assert x.shape[0] == len(y), f"x.shape = {x.shape}, len(y) = {len(y)}"
        ...
        if self.multiclass:
            # exp_classes is a list of arrays, one per class
            # Each array has shape (batch_size, n_features)
            # We need to extract the explanation for sample i using class y[i]
...
class lime_wrapper(explainer_wrapper):
    ...
    def __init__(self, model, lime_type, mode, multiclass=False, feature_names=None, num_samples=100,
        ...
        print(f'Initializing {lime_type} LIME explainer wrapper')
    ...
    def extract_att_tabular(self, exp, y):
        ...
        # if x.ndim == 1:
        # Single explanation
        if y is None or (isinstance(y, list) and y[0] is None):
    ...
    def extract_att_image(self, exp, y=None):
        ...
        if y is None or (isinstance(y, (list, tuple)) and y[0] is None):
            y = exp.top_labels[0]
...
class deepexplain_wrapper(explainer_wrapper):
    ...
    def __init__(self, model, method_type, mode, multiclass=False, feature_names=None,
                 class_names=None, train_data=None, num_features=None, categorical_features=None,
                 nsamples=100, verbose=False):
        print(f'Initializing "{method_type}" DeepExplain wrapper')
    ...
    def __call__(self, x, y=None, return_dict=False, x_raw=None, show_plot=False):
        ...
        # Handle TensorFlow 2.x vs 1.x compatibility
        try:
            session = K.get_session()
        except AttributeError:
            # TensorFlow 2.x - sessions are not used
            session = None
...
class gsenn_wrapper(explainer_wrapper):
    ...
    def __call__(self, x, y=None, return_dict=False, x_raw=None, show_plot=False):
        ...
        attrib_mat = self.net.thetas.cpu()

        nx, natt, nclass = attrib_mat.shape

        if y is None:
            # Will compute attribution w.r.t. predicted class
            vals, argmaxs = torch.max(pred, -1)
            attributions = attrib_mat.gather(2,argmaxs.view(-1,1).unsqueeze(2).repeat(1,natt,nclass))[:,:,0].cpu().numpy()
            ...
            # Select attributions for the specified class(es)
            if y.dim() == 0 or len(y) == 1:
                class_idx = y.item() if y.dim() == 0 else y[0].item()
        ...
        if show_plot:
            if self.input_type == 'image':
                ...
                if not isinstance(x_plot, np.ndarray):
...
try:
    ...
    withinterpretwoe = True
except Exception:
    withinterpretwoe = False
    print('interpretwoe not installed. Robust interpret wont be able to test WOE explainer')
